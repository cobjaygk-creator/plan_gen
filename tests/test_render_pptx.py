...
from tools.blocks import grid_block, new_highlight_block
...
def test_render_pptx_inserts_new_badge_and_skips_render_error_sections(tmp_path):
    new_items = [{"name": "NEW아이템", "image": None, "is_new": True}]
    rest_items = [{"name": f"일반아이템{i}", "image": None, "is_new": False} for i in range(3)]
    pages = new_highlight_block(new_items, rest_items)
